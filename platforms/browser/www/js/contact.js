...
function sendMail() {
	...
	var datos=  "idUser=" + Juser.result.idUser+ "&mail="+Juser.result.email+
	...
	if(mensaje !== "" && mensaje !== null){
		...
		$.ajax({	
			...
			success: function (data) {			
				...
				if (b.status === "ok"){
					state=1;
					myApp.alert('Su mensaje fué enviado exitosamente!', "Touch and Shop");
					mensajeInput.value="";
					asuntoInput.value="";					
				}		
			},error: function (data) {
	...
	myApp.alert('Debe ingresar un mensaje', "Touch and Shop");
	mensajeInput.value="";
	asuntoInput.value="";
	}
